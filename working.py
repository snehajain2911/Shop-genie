...
# === MongoDB Setup ===
uri = "mongodb+srv://abhidhanroy02072004:abhi1234@storedb.caiu1s9.mongodb.net/store_db?retryWrites=true&w=majority"
...
if user_input:
    with st.spinner("🤖 Understanding your need..."):
        ...
        if not items:
            ...
            st.subheader("🗺️ Store Map with Product Locations")
            ...
            for aisle, color in AISLE_COLORS.items():
                ...
                products_list = aisle_products.get(aisle, [])
                products_str = ', '.join(products_list) if products_list else "No products"
                st.markdown(f"<span style='color:{hex_color}'>🟢 Aisle {aisle}</span>: {products_str}", unsafe_allow_html=True)
            ...
            # === Generate QR Code with Localhost URL Placeholder ===
            pdf_url = "http://localhost:8501/shopping_summary.pdf"  # Change if hosted online
